...
const DEFAULT_PROVIDER_IDS: SearchProviderId[] = ['zlibrary'];

function normalizeProviderIds(providerIds: SearchProviderId[] | undefined): SearchProviderId[] {
	const input = providerIds && providerIds.length > 0 ? providerIds : DEFAULT_PROVIDER_IDS;
...
export class SearchBooksUseCase {
	...
	async execute(input: SearchBooksInput): Promise<ApiResult<SearchBooksResponse>> {
		...
		const requestedProviderIds = normalizeProviderIds(input.request.providers);
